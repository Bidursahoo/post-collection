...
// Types for the Post and User data structures
type Post = {
  id?: number; // Optional when creating a new post
  imageUrl: string;
  title: string;
  content: string;
...
// Define the function return types and parameter types
export async function getPosts(maxNumber?: number): Promise<Post[]> {
  ...
  await new Promise((resolve) => setTimeout(resolve, 1000)); // Simulate async behavior
  return maxNumber ? stmt.all(maxNumber) : stmt.all();
}

export async function storePost(
  post: Omit<Post, "id" | "createdAt" | "likes" | "isLiked">
